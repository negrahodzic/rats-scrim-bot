        ...
        server.config.scrims.forEach((scrim) => {
            if (msg.channel.id === scrim.channel_Waitlist) {
                if (msg.member.roles.cache.find(role => role.name === server.config.role_BotAdmin)) {
                    ...
                    msg.channel.updateOverwrite(msg.guild.roles.cache.find(role => role.name === scrim.role_Waitlist), { VIEW_MESSAGES: true, SEND_MESSAGES: true });
                    ...
                    msg.channel.send(scrim.text_open_waitlist);
                } else {
